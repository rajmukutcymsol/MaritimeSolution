...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/Customers/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Customers/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    ...
    if ($('#customer_name').val().trim() == "") {
        ...
        $('#customer_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
