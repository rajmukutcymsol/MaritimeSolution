...
function loadData() {
    $.get("/Developer/GetAll", function (data) {
...
function save() {
    ...
    var request = {
        ...
        developer_name: $("#employee_id option:selected").text(),
        is_active: is_active
    }
...
$('body').on('click', '.btnEdit', function () {
    $.get("/Developer/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Developer/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        alert(data.Title);
                        loadData();
                    });
...
function validate() {
    var isValid = true;
    if ($('#developer_name').val() == "")
    {
        $('#developer_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#developer_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
