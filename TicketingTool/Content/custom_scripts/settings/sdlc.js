...
function loadData() {
    $.get("/SDLC/GetAll", function (data) {
        $('#tbl_sdlc_status').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/SDLC/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/SDLC/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#sdlc_status').val().trim() == "") {
        $('#sdlc_status').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#sdlc_status').css('border-color', 'lightgrey');
    }
    return isValid;
}
