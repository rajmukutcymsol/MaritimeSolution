...
$(document).ready(function () {
    ...
    // Format the current date and set it as the value of the input
    $('#report_date').val(formatDate(currentDate));
    $('#date_of_action').val(formatDate(currentDate));
...
function loadData() {
    $.get("/InBound/GetGargoByID", { auto_id: $("#auto_id").val(), report_date: $("#report_date").val(), HH_report_date: $("#HH_report_date").val(), MM_report_date: $("#MM_report_date").val() }, function (data) {
...
function save_daily_report() {
    ...
    $.ajax({
        url: '/InBound/InsertDailyReport', // Replace with the actual URL
        data: JSON.stringify(request),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
...
$("#report_date").datepicker({
    onSelect: function (dateText, inst) {
        ...
        $.ajax({
            url: '/InBound/CreateDailyReportForDate', // Replace with the actual URL
            data: JSON.stringify(request),
            type: "POST",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            success: function (result) {
                console.log(result);
                if (result != null) {
                    $("#gang_previous").val(result.gang_previous);
                    $("#previous_hold1_out").val(result.previous_hold1_out);
                    $("#previous_hold2_out").val(result.previous_hold2_out);
                    $("#previous_hold3_out").val(result.previous_hold3_out);
                    $("#previous_hold4_out").val(result.previous_hold4_out);
                    $("#previous_hold5_out").val(result.previous_hold5_out);
                    $("#previous_total").val(result.previous_total);
                }

            },
            error: function (xhr, textStatus, errorThrown) {
                $("#gang_previous").val('');
                $("#previous_hold1_out").val('');
                $("#previous_hold2_out").val('');
                $("#previous_hold3_out").val('');
                $("#previous_hold4_out").val('');
                $("#previous_hold5_out").val('');
                $("#previous_total").val('');
            }
        });
    }
