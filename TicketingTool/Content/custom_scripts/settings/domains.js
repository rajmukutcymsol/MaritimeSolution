...
function loadData() {
    $.get("/Domains/GetAll", function (data) {
        $('#tblDomains').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/Domains/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Domains/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    ...
    if ($('#domain_name').val().trim() == "") {
        ...
        $('#domain_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
