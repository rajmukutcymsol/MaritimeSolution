...
function loadData() {
    $.get("/SolutionArchitect/GetAll", function (data) {
        $('#tbl_Data').DataTable({
            ...
            "columns": [
                ...
                { 'data': 'master_solution_architect_name' },
                {
                    data: "is_active",
                    "searchable": false,
                    "orderable": false,
...
function save() {
    ...
    var request = {
        ...
        employee_name : $("#employee_id option:selected").text(),
        is_active: is_active
    }

    $.ajax({
        url: '/SolutionArchitect/Save',
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/SolutionArchitect/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/SolutionArchitect/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        alert(data.Title);
                        loadData();
                    });
