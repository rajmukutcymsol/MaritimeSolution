...
function loadData() {
    $.get("/State/GetAll", function (data) {
        $('#tbl_Data').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/State/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/State/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
