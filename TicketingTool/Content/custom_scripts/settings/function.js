...
function loadData() {
    $.get("/Functions/GetAll", function (data) {
        $('#tbl_Function').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/Functions/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Functions/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#function_name').val().trim() == "") {
        $('#function_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#function_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
