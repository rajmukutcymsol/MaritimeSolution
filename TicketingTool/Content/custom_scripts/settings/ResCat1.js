...
function loadData() {
    $.get("/ResCat1/GetAll", function (data) {
        $('#tbl_ResCat1').DataTable({
...
function save() {
    ...
    var request = {
        res_cat1_name: $("#res_cat1_name").val(),
        is_active: is_active
    }
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/ResCat1/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
        $("#res_cat1_name").val(data.res_cat1_name);
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/ResCat1/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#res_cat1_name').val() == "") {
        $('#res_cat1_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#res_cat1_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
