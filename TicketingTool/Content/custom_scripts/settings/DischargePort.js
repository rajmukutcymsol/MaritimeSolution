...
function loadData() {
    $.get("/DischargePort/GetAll", function (data) {
        $('#tbl_cliui').DataTable({
...
function save() {
    ...
    var request = {
        discharge_port_name: $("#discharge_port_name").val(),
        is_active: is_active
    }

    $.ajax({
        url: '/DischargePort/Save',
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/DischargePort/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
        $("#discharge_port_name").val(data.discharge_port_name);
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/DischargePort/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#discharge_port_name').val().trim() == "") {
        $('#discharge_port_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#discharge_port_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
