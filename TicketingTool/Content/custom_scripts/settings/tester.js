...
function loadData() {
    $.get("/Tester/GetAll", function (data) {
        $('#tbl_Tester').DataTable({
...
function save() {
    ...
    var request = {
        ...
        tester_name: $("#employee_id option:selected").text(),
        is_active: is_active
    }
    //console.log(request);
    $.ajax({
...
$('body').on('click', '.btnEdit', function () {
    $.get("/Tester/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Tester/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                       // alert(data.Title);
                        loadData();
                    });
