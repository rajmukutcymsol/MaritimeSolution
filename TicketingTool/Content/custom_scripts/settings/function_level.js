...
function loadData() {
    $.get("/FunctionLevel/GetAll", function (data) {
        $('#tbl_FunctionLevel').DataTable({
...
function save() {
    ...
    var request = {
        function_level: $("#function_level").val(),
        is_active: is_active
    }

    $.ajax({
        url: '/FunctionLevel/Save',
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/FunctionLevel/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/FunctionLevel/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
