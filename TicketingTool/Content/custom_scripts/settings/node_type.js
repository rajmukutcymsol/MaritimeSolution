...
function loadData() {
    $.get("/NodeType/GetAll", function (data) {
        $('#tbl_NodeType').DataTable({
...
function save() {
    ...
    var request = {
        node_type_name: $("#node_type_name").val(),
        is_active: is_active
    }
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/NodeType/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
        $("#node_type_name").val(data.node_type_name);
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/NodeType/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#node_type_name').val().trim() == "") {
        $('#node_type_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#node_type_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
