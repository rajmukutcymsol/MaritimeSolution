...
function loadData() {
    $.get("/SolutionsTool/GetAll", function (data) {
        $('#tbl_SolutionTools').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/SolutionsTool/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/SolutionsTool/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#tool_name').val().trim() == "") {
        $('#tool_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#tool_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
