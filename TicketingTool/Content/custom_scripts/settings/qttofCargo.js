...
function loadData() {
    $.get("/QttofCargo/GetAll", function (data) {
        $('#tbl_Data').DataTable({
            "processing": true,
            "serverSide": false,
            "filter": true,
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/QttofCargo/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/QttofCargo/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
