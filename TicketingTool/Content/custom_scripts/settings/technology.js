...
function loadData() {
    $.get("/Technology/GetAll", function (data) {
        $('#tbl_Technology').DataTable({
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/Technology/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
        $("#technology_name").val(data.technology_name);
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Technology/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    ...
    if ($('#technology_name').val().trim() == "") {
        $('#technology_name').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#technology_name').css('border-color', 'lightgrey');
    }
    return isValid;
}