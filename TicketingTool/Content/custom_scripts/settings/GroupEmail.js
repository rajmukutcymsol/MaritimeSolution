...
function loadData() {
    $.get("/GroupEmail/GetAll", function (data) {
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/GroupEmail/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/GroupEmail/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    ...
    if ($('#group_name').val().trim() == "") {
        ...
        $('#group_name').css('border-color', 'lightgrey');
    }
    return isValid;
}
