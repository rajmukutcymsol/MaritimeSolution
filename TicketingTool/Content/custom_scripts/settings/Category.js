    ...
    $.get("/Category/GetAll", function (data) {
        $('#tbl_Categorytype').DataTable({
            ...
            "columns": [
                ...
                { 'data': 'category_name' },
                { 'data': 'category_name_discription' },
                {
                    data: "is_active",
                    "searchable": false,
                    "orderable": false,
...
function save() {
    ...
    var request = {
        ...
        category_name_discription: $("#category_name_discription").val(),
        is_active: is_active
    }
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/Category/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
        $("#category_name").val(data.category_name);
        $("#category_name_discription").val(data.category_name_discription);
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/Category/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
