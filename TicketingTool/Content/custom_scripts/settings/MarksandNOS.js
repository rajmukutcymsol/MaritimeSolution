...
function loadData() {
    $.get("/MarksAndNos/GetAll", function (data) {
        $('#tbl_cliui').DataTable({
...
function save() {
    ...
    var request = {
        marks_and_nos_name: $("#marks_and_nos_name").val(),
        
        is_active: is_active
    }

    $.ajax({
        url: '/MarksAndNos/Save',
...
$('body').on('click', '.btnEdit', function () {
    ...
    $.get("/MarksAndNos/GetById", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
...
$('body').on('click', '.btnDelete', function () {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/MarksAndNos/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
                    swal(data.Title, data.Message, data.Type).then(function () {
                        loadData();
                    });
                });
...
function validate() {
    var isValid = true;
    if ($('#marks_and_nos_name').val().trim() == "") {
