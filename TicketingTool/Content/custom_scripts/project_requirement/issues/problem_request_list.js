...
$(document).ready(function () {
    ...
    $('#request_status').change(function () {
        ...
        if (selectedValues == "00000000-0000-0000-0000-000000000000") {
            ...
            $.ajax({
                url: "/IssueRequest/GetAll_Filter",
                    ...
                    if (isTru) {
                        $('#tblNewRequest').DataTable({
                            ...
                            "columns": [
                                {
                                    ...
                                    "render": function (data, type, row, meta) {
                                        return '<input type="hidden" value=' + data + '><a href="/IssueRequest/Edit/' + data + '" style="margin-right: 5px;"><i class="fa fa-edit"></i></a>| &nbsp <a href="#" class="btnDelete" style="margin-right: 5px;"><i class="fa fa-trash"></i></a>';
                                    },
                                },
                                { 'data': 'auto_id' },
                                { 'data': 'project_name' },
                            ...
                            "initComplete": function () {
                                ...
                                var columnEmployeeName = api.column(8); // "employee_name" column
                                var footerEmployeeName = $(columnEmployeeName.footer());
                                footerEmployeeName.html('<label for="employee_name"></label>');
                                var selectFooterEmployeeName = $('<select style="color:#666060;" id="employee_name"></select>').appendTo(footerEmployeeName);

                                var uniqueValuesEmployeeName = columnEmployeeName.data().unique().sort().toArray();

                                selectFooterEmployeeName.append('<option value="">All</option>'); // Add "All" option on top

                                $.each(uniqueValuesEmployeeName, function (index, value) {
                                    selectFooterEmployeeName.append('<option value="' + value + '">' + value + '</option>');
                                });

                                selectFooterEmployeeName.on('change', function () {
                                    var val = $.fn.dataTable.util.escapeRegex($(this).val());
                                    columnEmployeeName.search(val ? '^' + val + '$' : '', true, false).draw();
                                });
                            ...
                            "columns": [
                                {
                                    ...
                                    "orderable": false,
                                    "render": function (data, type, row, meta) {
                                        return '<input type="hidden" value=' + data + '> <a href="/IssueRequest/OthersEdit/' + data + '" style="margin-right: 5px;"><i class="fa fa-edit"></i></a>';

                                        //return '<a href="/IssueRequest/Edit/' + data + '" style="margin-right: 5px;"><i class="fa fa-edit"></i></a>';
                                    },
                                },
                                { 'data': 'auto_id' },
                                { 'data': 'project_name' },
...
$('body').on('click', '.btnDelete', function () {
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get("/IssueRequest/Delete", { id: $(this).closest('tr').find('td:eq(0) input').val() }, function (data) {
